---
title: L3 统一运行时
aside: false
---

# L3 统一运行时

> **运行时（Runtime）的核心职责不是替智能体做计划，而是把真实执行变成持久、可恢复、可重建的事实。**

<iframe class="architecture-frame" src="../diagrams/layers/l3-runtime.architecture.html?embed=1" title="L3 统一运行时架构图"></iframe>

<div class="architecture-links">
  <a href="../diagrams/layers/l3-runtime.architecture.html">打开独立交互图 ↗</a>
  <a href="./agent-platform#_5-核心领域模型">阅读核心领域模型 →</a>
</div>

## 图中要点

- **三个时间尺度**：工作单元（Work）承载长期目标，会话（Session）表示智能体与工作单元的相对连续关系，运行（Run）表示一次连续执行片段。
- **事实与投影分离**：事件（Event）与结果（Result）是事实；状态（State）与观察（Observation）是从事实构建出的视图。
- **长期连续性属于工作单元**：智能体可以忘记、退出或替换；事件、制品、工作空间与恢复机制让工作单元继续。
