...
| State | 状态 | 泛称；描述 L3 时应优先使用更精确的“运行时投影状态”等限定术语 |
| Runtime Projection State | 运行时投影状态 | 从执行事实得到的当前运行视图，用于查询、调度、治理和展示；可由事实重建 |
...
| Work Environment State | 工作环境状态 | 工作空间、沙箱、浏览器、Git、外部资源等执行世界状态；不等同于运行时投影状态 |
| Command | 命令 | 表示希望发生什么 |
...
| Recovery | 恢复 | 直接使用中文 |
| Handoff | 交接 | 直接使用中文 |
| Event Log | 事件日志 | 记录执行事实；是执行历史的重要事实源，不等同于整个外部世界的事实源 |
