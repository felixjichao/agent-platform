...
features:
  - title: 运行时管事实
    details: 工作单元、会话、运行、事件、状态、动作、结果与恢复构成稳定的执行事实。
  - title: 执行框架管策略
    details: 执行框架负责策略控制，工作流、智能体循环、PTC、多智能体等策略可以持续替换；运行时负责稳定的执行生命周期。
  - title: 平台管边界
...
3. **执行框架管策略控制，运行时管执行生命周期**：可演进的决策策略与稳定的执行、等待、恢复和终止语义必须解耦。
4. **工作单元承载长期连续性，而不是智能体**：智能体可以退出、遗忘或替换，但工作单元必须持续存在。
...
10. **多智能体的关键是工作前沿**：平台真正需要调度的是可独立推进的工作项，而不是智能体数量。
11. **评估是能力规格**：执行框架说明怎么实现，评估说明怎么证明具备能力。
12. **智能体安全的本质是可达范围控制**：通过确定性边界限制每个工作单元、运行、子智能体可触达的世界。
...
<div class="diagram-grid">
  <a class="diagram-card" href="./architecture/l4-strategy-harness">
    ...
    <p>执行框架决定下一步怎么推进；工作流、智能体循环、PTC、多智能体是不同策略模式。纯策略切换留在 L4，独立执行生命周期由 L3 管理。</p>
  </a>
  <a class="diagram-card" href="./architecture/l3-runtime">
    <span class="diagram-card__eyebrow">L3 · 执行事实</span>
    <strong>统一运行时</strong>
    <p>工作单元、会话、运行、事件、状态如何组成持久、可恢复的执行事实，并记录人工中断、纠偏与升级。</p>
  </a>
