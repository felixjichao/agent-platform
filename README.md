# Agent Platform

> **Agent Platform 的核心，不是“跑一个智能体循环”，而是把动态组装、非确定、长期、并行、可编程的智能执行过程，约束在持久、可恢复、可验证、可观测、可治理、能够持续演进的生产系统中。**

**在线阅读：<https://felixjichao.github.io/agent-platform/>**

## 为什么需要 Agent Platform

过去我们很容易这样理解智能体：

```text
提示词
  ↓
LLM
  ↓
工具调用
  ↓
结果
  ↓
LLM
  ↓
...
```

这个最小循环足够解释一个智能体 Demo，却远远不足以解释一个生产级智能体系统。

当智能体开始长时间运行、调用越来越多的工具、操作真实业务系统、处理中间文件和大量数据、跨多个会话延续工作、动态创建子智能体、执行生成代码并自主规划和恢复时，真正困难的问题就不再是：

> **怎么让模型调用工具？**

而是：

> **怎么治理一个非确定的智能执行过程？**

本项目希望建立一套足够稳定的智能体软件架构，使对话助手、科研助手、知识库、工作流、编程助手、自主智能体等不同产品形态，可以共享同一套底层执行基础设施。

---

# 核心判断

下面是当前研究形成的 13 个核心架构判断。

## 01. 应用形态不等于执行范式

对话、科研助手、知识库、编程助手、行业应用，是产品形态。

直接执行、工作流、智能体循环、程序化工具调用（PTC）、多智能体，是执行范式。

```text
产品形态
对话 · 科研 · 知识库 · 编程助手
              │
              │ 不等于
              ▼
执行范式
直接执行 · 工作流 · 智能体循环 · PTC · 多智能体
```

同一个产品可以混合多种执行策略，因此不应该为每一种智能体产品重新实现一套运行时。

## 02. 工作流与智能体的真正区别，是谁拥有执行控制权

工作流中，下一步主要由预定义代码或图决定；智能体中，下一步主要由模型根据当前上下文动态决定。

```text
静态工作流
    ↓
动态工作流 / 计划
    ↓
智能体循环
```

二者更像一条连续谱，而不是截然分开的两套技术。

> **工作流图是输入表达；智能体图更接近运行时结果。**

## 03. 运行时管事实，执行框架管策略

这是整套架构最重要的边界之一。

```text
业务层
→ 要完成什么

执行框架
→ 当前准备怎么完成

运行时
→ 执行生命周期如何推进、实际发生了什么

工作环境
→ 动作如何真正作用于世界
```

规划、待办、上下文策略、内部路由、委派与完成策略，都可能随着模型能力提升而变化；这些属于可替换的策略实现。

而工作单元（Work）、会话（Session）、运行（Run）、事件（Event）、状态、动作（Action）、结果（Result）、制品（Artifact）、恢复这些执行事实应该保持稳定。

> **执行框架负责策略控制，运行时负责执行生命周期控制；运行时应稳定，执行框架应可替换、可简化，甚至最终消失。**

## 04. 工作单元承载长期连续性，而不是智能体

一个长期任务不应该依赖“这个智能体永远不能忘记”。真正持久的应该是：

```text
工作单元
├── 目标
├── 任务
├── 共享资源
├── 工作前沿
├── 制品
├── 工作空间
├── 进度
└── 会话
```

智能体可以丢失上下文、结束会话、更换模型或执行框架，甚至被新的智能体接管，但工作单元必须继续存在。

> **智能体可以忘记、退出或者被替换；工作单元必须记住。**

## 05. 事件是事实，状态和上下文都是投影

统一运行时的基本关系：

```text
动作 / 结果
    ↓
   事件
    ↓
 事件日志
    ↓
   状态
```

其中：

```text
事件   → 已经发生的事实
状态   → 根据事实计算出的当前视图
上下文 → 当前模型实际需要看到的工作集
```

因此：

> **事件是事实，状态是投影，上下文是查询结果。**

快照、摘要、上下文压缩都不能反过来成为真实历史。

## 06. 上下文是工作集，不是整个世界

生产级智能体不应该尝试把所有信息永久塞进上下文窗口。

更合理的模型是：

```text
会话
状态
记忆
笔记
制品
工作空间
技能
能力
外部资源
     ↓
上下文构建器
     ↓
   上下文
```

上下文应该可以丢弃、可以重建，并按照当前目标动态投影、按需加载知识和能力。

> **上下文包含的是地图，而不是整个世界。**

模型需要的是当前工作的地图，而不是整个世界。

## 07. LLM 应该是控制面，而不是数据面

很多智能体系统让模型同时承担推理、编排和数据搬运，结果大量 Token 被消耗在循环、过滤、关联、聚合和格式转换上。

更合理的是：

```text
智能体 / 执行框架
= 执行控制面

智能体工作环境
= 数据面 + 动作面
```

程序化工具调用（PTC）体现了这种方向：

```text
模型
 ↓ 生成程序
程序
 ├── 工具 A
 ├── 工具 B
 ├── 循环
 ├── 过滤
 └── 聚合
 ↓
观察
 ↓
模型
```

> **一个模型推理步骤，可以展开成多个受治理的环境动作。**

## 08. 能力、工具、技能、MCP 是不同抽象

```text
能力
→ 系统能做什么稳定工作

工具
→ 执行框架 / 模型怎样表达能力调用

技能
→ 这类任务通常应该怎么做

MCP / API
→ 具体系统怎样被连接
```

真正的设计单位是对执行策略有意义、值得独立选择和治理的**能力语义**，而不是后端 API 端点或 MCP Tool List。

```text
能力定义
   ↓
能力发现 → 当前能力视图 → L4
   ↓
能力解析 → 当前能力绑定 → L3
   ↓
能力适配器
   ↓
MCP / API / SDK / 本地实现
```

因此：

> **MCP 是连接标准，不是能力设计标准；Tool Call 也不等于 Runtime Action。**

技能则是可按需加载的程序性知识包，可以指导如何组合能力，但不会自动增加权限或资源可达范围。

## 09. 智能体本身可以是动态组装出来的

随着技能、工具、知识、上下文都可以按需加载：

```text
基础智能体
+ 选中的技能
+ 选中的工具
+ 召回的知识
+ 任务上下文
       ↓
实际工作的智能体
```

因此不必为每一种业务场景预先创建一个固定智能体。平台更应该建设稳定能力语义、能力发现 / 解析和运行时组合，而不是不断增加静态智能体类型。

> **智能体使用能力，但不应该拥有能力。**

## 10. 多智能体的关键不是智能体数量，而是工作前沿

复杂任务并不自动意味着需要多智能体。只有当任务具备可并行、上下文相对独立、结果可合并、依赖密度较低等特征时，多智能体才真正产生价值。

真正决定并行度的是：

> **当前有多少个可以独立领取、独立推进、独立验证的工作项（Work Item）。**

这些工作项共同构成工作前沿（Work Frontier）。

> **多智能体的核心是工作调度器，而不是智能体调度器。**

同时：

```text
委派
→ 传递目标

授权
→ 单独推导最小权限
```

> **委派不应该默认复制权限。**

## 11. 评估不是测试附件，而是能力规格

对非确定智能体，仅仅写一个预期输出通常不够。更合理的基本单位是试验（Trial）：

```text
任务
 ↓
试验
├── 执行框架修订版本
├── 环境
├── 追踪
├── 结果
└── 评分结果
```

必须区分：

```text
追踪 → 智能体是怎么做的
结果 → 最终真实结果是什么
```

能力契约同时包含目标、验收标准、质量标准、评估契约和证据要求。

> **执行框架说明怎么实现；评估说明怎么证明。**

能力评估用于探索能力边界，回归评估用于守住已经获得的能力。

## 12. 智能体安全的本质，是控制智能体能触达到多大的世界

模型变得更可靠，并不意味着系统自动更安全。

```text
部署风险
≈ 失败概率 × 影响半径
```

提示词、模型训练、分类器、HITL 可以降低失败概率；真正限制最大后果的，是确定性边界：

```text
文件系统
网络
凭据
能力
数据
记忆
其他智能体
外部系统
```

因此：

> **隔离控制的本质是可达范围控制。**

每个工作单元、运行、子智能体都应该拥有尽可能小的可达世界。

最终授权也不应该只是 `github_access = true`，而应该围绕：

```text
主体
× 委派
× 能力
× 资源
× 范围
× 时长
× 工作单元上下文
```

构建。

> **治理决定边界画在哪里；基础设施保证智能体越不过这条边界。**

## 13. 自主性不是模型属性，而是部署治理结果

同一个模型在不同执行框架、权限范围、风险边界、产品默认设置和人工监督方式下，会表现出完全不同的实际自主性。

```text
执行策略
→ 当前任务怎么推进

自主性
→ 在既定边界内，智能体可以自己决定多少
```

因此，自主性不应该被建模成固定等级。系统应根据风险、可逆性、可达范围、验证能力和人工监督能力动态分配自主空间。纯策略切换属于执行框架内部行为；人工中断、纠偏、审批和执行生命周期转移等会改变可靠执行语义的控制变化才需要进入运行时或治理事实。

> **生产级智能体的目标不是最大化自主性，而是在可验证、可观察、可干预的边界内，最大化有用的自主性。**

---

# 智能体软件栈

上述原则最终形成了当前的七层软件栈：

```text
┌───────────────────────────────────────────────┐
│ L7 业务 / 产品                                │
│ 目标 · 项目 · 业务任务                        │
├───────────────────────────────────────────────┤
│ L6 能力契约                                   │
│ 验收标准 · 质量标准 · 评估契约                │
├───────────────────────────────────────────────┤
│ L5 能力工程                                   │
│ 能力定义 · 能力发现 · 能力解析                │
│ 工具 · 技能 · 能力绑定                        │
├───────────────────────────────────────────────┤
│ L4 执行策略                                   │
│ 工作流 · 智能体循环 · PTC · 多智能体          │
├───────────────────────────────────────────────┤
│ L3 统一运行时                                 │
│ 工作单元 · 会话 · 运行 · 事件 · 状态          │
│ 动作 · 观察 · 制品 · 恢复                      │
├───────────────────────────────────────────────┤
│ L2 智能体工作环境                             │
│ 工作空间 · 执行 · 验证                        │
│ 能力代理 · 沙箱                               │
├───────────────────────────────────────────────┤
│ L1 基础设施 / 真实世界                        │
│ API · SaaS · 数据库 · 浏览器 · 文件 · 计算    │
└───────────────────────────────────────────────┘
```

横向贯穿整个软件栈：

```text
上下文 / 记忆
评估
可观测性 / 追踪
安全 / 治理
人工监督
注册表 / 版本管理
预算 / 成本
```

每层只回答一个核心问题：

| 层 | 核心问题 |
|---|---|
| 业务 / 产品 | 要完成什么？ |
| 能力契约 | 什么叫真正具备这项能力？ |
| 能力工程 | 系统有哪些能力，当前怎样被发现和实现？ |
| 执行策略 | 当前任务怎么推进？ |
| 统一运行时 | 实际发生了什么？ |
| 工作环境 | 智能体能观察和作用于什么世界？ |
| 基础设施 | 真正的资源在哪里？ |

---

# 一个更完整的智能体心智模型

生产级智能体不再只是：

```text
LLM + 工具 + 循环
```

而更接近：

```text
业务目标
   ↓
能力契约
   ↓
动态能力组装
   ↓
执行框架 / 执行策略
   ↓
统一运行时
   ↓
智能体工作环境
   ↓
真实世界
```

同时，上下文 / 记忆、评估 / 验证、追踪 / 可观测性、安全 / 治理、人工监督贯穿整个执行过程。

> **Agent Platform 本质上是一套智能执行基础设施。**

---

# 为什么做这个项目

智能体领域正在快速出现新的概念：智能体循环、工作流、执行框架、上下文工程、记忆、技能、工具、MCP、PTC、多智能体、评估、沙箱、智能体身份、自主性、人工监督。

单独理解每个概念并不困难。困难的是：

> **它们究竟属于哪一层？彼此是什么关系？什么应该成为稳定平台抽象，什么只是当前模型时代的临时脚手架？**

本项目通过持续阅读和分析智能体工程实践，逐步建立统一的平台架构。

不是简单整理文章来源，而是保留：

```text
文章来源
   ↓
逐节分析
   ↓
架构推导
   ↓
Agent Platform 演进
```

整个过程。

---

# 研究路线

目前已经系统分析：

1. [Building effective agents](articles/01-building-effective-agents/README.md)
2. [Scaling Managed Agents: Decoupling the brain from the hands](articles/02-scaling-managed-agents/README.md)
3. [Effective context engineering for AI agents](articles/03-effective-context-engineering/README.md)
4. [Effective harnesses for long-running agents](articles/04-effective-harnesses-for-long-running-agents/README.md)
5. [Harness design for long-running application development](articles/05-harness-design-for-long-running-apps/README.md)
6. [How we built our multi-agent research system](articles/06-multi-agent-research-system/README.md)
7. [Building a C compiler with a team of parallel Claudes](articles/07-building-c-compiler/README.md)
8. [Demystifying evals for AI agents](articles/08-demystifying-evals-for-ai-agents/README.md)
9. [Writing effective tools for agents — with agents](articles/09-writing-effective-tools-for-agents/README.md)
10. [Equipping agents for the real world with Agent Skills](articles/10-agent-skills/README.md)
11. [Code execution with MCP: Building more efficient agents](articles/11-code-execution-with-mcp/README.md)
12. [How we contain Claude across products](articles/12-how-we-contain-claude/README.md)
13. [Measuring AI agent autonomy in practice](articles/13-measuring-agent-autonomy/README.md)

每一篇文章一个目录，每一个核心问题独立成节。

---

# 深入阅读

## 总体架构

[**智能体平台总体架构 →**](architecture/agent-platform.md)

这是当前 Agent Platform 的规范性架构文档，包含：

- 七层软件栈；
- 工作单元 / 会话 / 运行领域模型；
- 事件 / 状态；
- 上下文 / 记忆 / 工作空间；
- 技能 / 工具 / MCP；
- PTC；
- 多智能体；
- 评估 / 验证；
- 安全 / 治理；
- 自主性 / 人工监督 / 部署后监控；
- 可观测性；
- 能力生命周期。

## 查看架构如何一步步形成

每完成一篇文章，都采用：

```text
文章研究提交
       ↓
架构吸收 / 修正
       ↓
架构演进提交
```

因此可以直接：

```bash
git log -- architecture/agent-platform.md
```

查看整个架构从执行控制、托管智能体、上下文工程、长时运行智能体、多智能体、智能体工作环境、评估、能力工程、PTC、安全 / 治理到自主性与人工监督逐步形成的过程。

## 写作规范

- 正文尽可能使用中文；
- 重要英文术语首次出现时保留原词，之后优先使用中文；
- 目录名、文件名、接口、事件类型和代码标识符保持英文；
- 不把我们的架构推导伪装成文章来源；
- 一节只处理一个核心问题。

完整规范见 [docs-style-guide.md](docs-style-guide.md)，统一术语见 [terminology.md](terminology.md)。

---

# 一句话总结

> **不要把 Agent Platform 理解成一个更复杂的智能体循环。**
>
> **它真正要解决的是：如何把非确定的智能执行，变成可以进入生产环境的软件系统。**